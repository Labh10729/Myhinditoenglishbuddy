...
# Initialize Bot and Flask Web Server
...
def translate_to_english_quiz(hindi_text):
    print("⏳ Translating...")
    system_prompt = """
    Translate this Hindi quiz to English.
    ...
    # Using a super-fast model instead of the slow R1 model
    response = client.chat.completions.create(
        ...
        temperature=0.1,
        max_tokens=250 # Forces the AI to be quick
    )
...
def send_welcome(message):
    bot.reply_to(message, "👋 Send me a Hindi quiz with options.\nI will translate it fast, ask for the answer, and create a Poll!")

# Step A: Receive Hindi text, Translate, and Ask for Answer
...
def handle_correct_answer(message):
    ...
    if not message.text.isdigit():
        bot.reply_to(message, "❌ Please reply with a number (like 1 or 2).")
        return
    ...
    try:
        ...
        # Clear memory so user can send a new quiz
